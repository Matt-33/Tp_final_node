...
export const restaurantsController = {
	...
	getById: async (req: Request, res: Response) => {
		try {
			...
			console.error(
				"Erreur lors de la récupération du restaurant:",
				error
			);
			res.status(500).json({
				success: false,
	...
	getAvailableTables: async (req: Request, res: Response) => {
		try {
			...
			const tables = await tableModels.getAvailableByRestaurantId(
				restaurantId
			);
			...
			console.error(
				"Erreur lors de la récupération des tables disponibles:",
				error
			);
			res.status(500).json({
				success: false,
	...
	getMenuCategories: async (req: Request, res: Response) => {
		try {
			...
			const categories = await menuCategoriesModel.getByRestaurantId(
				restaurantId
			);
	...
	getReservations: async (req: Request, res: Response) => {
		try {
			...
			if (date) {
				...
				reservations = await reservationsModel.getByRestaurantId(
					restaurantId
				);
			}
	...
	getUpcomingReservations: async (req: Request, res: Response) => {
		try {
			...
			const reservations =
				await reservationsModel.getUpcomingByRestaurant(restaurantId);
	...
	getAverageRating: async (req: Request, res: Response) => {
		try {
			...
			const ratingData = await reviewsModel.getAverageRatingByRestaurant(
				restaurantId
			);
